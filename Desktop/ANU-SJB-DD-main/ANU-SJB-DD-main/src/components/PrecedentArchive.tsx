...
export default function PrecedentArchive() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Precedent[];
      setPrecedents(data);
      setLoading(false);
    }, (error) => {
    ...
    <div className="space-y-12">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`px-4 sm:px-6 py-2 sm:py-3 rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all border-2 ${
        ...
        {allTags.map(tag => (
          <button
            ...
            className={`px-4 sm:px-6 py-2 sm:py-3 rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all border-2 flex items-center gap-2 ${
        ...
        {loading ? (
          ...
          filtered.map((precedent) => (
            <motion.div
              key={precedent.id}
              layout
              className="bg-white dark:bg-slate-900 rounded-[2rem] sm:rounded-[2.5rem] border border-slate-200 dark:border-slate-800 p-6 sm:p-12 hover:border-emerald-500/50 transition-all group relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity text-slate-400 dark:text-slate-100">
                <BookOpen size={100} className="sm:w-[160px] sm:h-[160px]" />
              </div>
              ...
              <h3 className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white mb-4 sm:mb-6 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors uppercase tracking-tight leading-none relative z-10">
                {precedent.title}
              </h3>
              
              <p className="text-slate-500 dark:text-slate-400 text-sm sm:text-base leading-relaxed mb-6 sm:mb-10 flex-grow font-medium relative z-10 line-clamp-4">
                {precedent.summary}
              </p>

              <div className="flex flex-wrap gap-2 mb-8 sm:mb-12 relative z-10">
                {precedent.tags?.map(t => (
                  <span key={t} className="text-[8px] sm:text-[10px] font-black text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-3 sm:px-4 py-1 sm:py-1.5 rounded-xl uppercase tracking-widest border border-emerald-100 dark:border-emerald-900">
                    {t}
                  </span>
                ))}
              </div>
              ...
              <button className="w-full py-4 sm:py-5 bg-slate-50 dark:bg-slate-800 hover:bg-slate-900 dark:hover:bg-emerald-600 text-slate-900 dark:text-slate-200 hover:text-white rounded-2xl font-black uppercase tracking-widest text-[9px] sm:text-[10px] transition-all flex items-center justify-center gap-3 border border-slate-100 dark:border-slate-700 active:scale-95 relative z-10">
                ...
                <ExternalLink size={14} className="opacity-40 sm:w-[16px] sm:h-[16px]" />
              </button>
            </motion.div>
          ))
        ) : (
